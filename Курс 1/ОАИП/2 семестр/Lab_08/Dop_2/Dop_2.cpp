...
bool isNull() 
{
    return (head == nullptr);  // Проверка, пуста ли очередь
}
...
void insertWithPriority(double x) 
{
    Item* p = new Item;
    p->data = x;
    p->next = nullptr;

    if (isNull())  // Вставка в пустую очередь
    {
        head = tail = p;  
    }
...
void clearQueue() 
{
    while (!isNull()) 
    {
        deleteFirst(); // Очистка очереди путем удаления всех элементов
    }
}
...
int main() 
{
    ...
    while (choice != 0) 
    {
        ...
        case 2:
            ...
            break;
        case 3:
            deleteFirst(); // Извлечение элемента из начала очереди
            break;
        case 4:
