...
namespace Sportics.ViewModel
{
    public class RegistrationViewModel : BaseViewModel, IDataErrorInfo
    {
        ...
        private void Register()
        {
            ...
            if (!UserRepository.CheckEmailAndPhoneNumber(Email, Phone))
            {
                ShowMessage("Почта (телефон) уже зарегистрирована");
                return;
            }

            UserRepository.AddUser(Name, Email, Phone, Password);

            User user = UserRepository.SelectUser(Email, Password);
