...
namespace Sportics.ViewModel
{
    public class AccountViewModel : BaseViewModel
    {
        ...
        private void LoadUserSchedules()
        {
            ...
            var allRecords = ClientSessionRepository.GetAllClientSessionRecords()
            ...
            foreach (var record in allRecords)
            {
                ...
                {
                    ClientSessionRepository.DeleteClientSessionRecord(record); // Удаляем запись через день после занятия
                }
            }

            // Загружаем актуальные занятия после удаления старых
            var schedules = UserRepository.LoadUserSchedules(userId);
            UserSchedules = new ObservableCollection<Schedule>(schedules);

            UserMemberships = new ObservableCollection<UserMembershipInfo>(
                UserRepository.GetUserMemberships(Session.CurrentUser));
        ...
        private void CancelSchedule(object obj)
        {
            ...
            {
                bool success = UserRepository.CancelUserSchedule(Session.CurrentUser.Id, schedule.Id);
                if (success)
                {
        ...
        private void CancelMembership(object obj)
        {
            if (obj is UserMembershipInfo membershipInfo)
            {
                ...
                var membershipOrder = MembershipOrderRepository
                ...
                MembershipOrderRepository.DeleteOrder(membershipOrder.Id);
