...
namespace Sportics.Model
{
    public static class DataWorker
    {
        ...
        public static Membership AddMembership(string fullName, string shortName, string category, string description, decimal price, byte[] photo, int durationInDays)
        {
            using (ApplicationContext db = new ApplicationContext())
            {
        ...
        public static void DeleteMembership(Membership membership)
        {
            using (ApplicationContext db = new ApplicationContext())
            {
                var toDelete = db.Memberships.Find(membership.Id);
                if (toDelete != null)
                {
                    db.Memberships.Remove(toDelete);
                    db.SaveChanges();
                }
            }
        }
