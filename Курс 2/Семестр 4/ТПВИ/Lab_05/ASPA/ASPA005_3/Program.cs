...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        app.MapDelete("/A/{x:int:min(1)}-{y:int:min(1):max(100)}", (HttpContext context, [FromRoute] int x, [FromRoute] int y) => Results.Ok(new { path = context.Request.Path.Value, x = x, y = y }));
        //--- B --------
