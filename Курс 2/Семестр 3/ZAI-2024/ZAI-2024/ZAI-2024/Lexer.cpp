...
namespace Lex
{
	LEX lexAnaliz(Log::LOG log, In::IN in)
	{
		...
		for (i = 0; word[i][0] != NULL; indexLex++, i++)
		{
			...
			FST::FST fstTypeShort(word[i], FST_SHORT);
			if (FST::execute(fstTypeShort))
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_SHORT, LT_TI_NULLIDX, line);
				LT::Add(lextable, entryLT);

				newindf = true;
				entryIT.iddatatype = IT::SHR;
				continue;
			}
			...
			{
				if (_mbscmp((unsigned char*)word[i], (unsigned char*)"char") == 0) 
				{
			...
			{
				if (_mbscmp((unsigned char*)word[i], (unsigned char*)"bool") == 0)
				{
			...
			FST::FST fstShowline(word[i], FST_SHOWLINE);
			if (FST::execute(fstShowline))
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_SHOWLINE, LT_TI_NULLIDX, line);
				LT::Add(lextable, entryLT);
				continue;
			}

			FST::FST fstAsk(word[i], FST_ASK);
			if (FST::execute(fstAsk))
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_ASK, LT_TI_NULLIDX, line);
				LT::Add(lextable, entryLT);
				continue;
			}
			...
			FST::FST fstTrue(word[i], FST_TRUE);
			if (FST::execute(fstTrue))
			{
				bool value = true;

				for (int k = 0; k < idtable.size; k++)
				{
					if (idtable.table[k].iddatatype == IT::IDDATATYPE::BOOL &&
						idtable.table[k].value.vbool == value && idtable.table[k].idtype == IT::L)
					{
			...
			FST::FST fstFalse(word[i], FST_FALSE);
			if (FST::execute(fstFalse))
			{
				bool value = false;
			...
			FST::FST fstLiteralInt(word[i], FST_SHRLIT);
			if (FST::execute(fstLiteralInt))
			{
				...
				for (int k = 0; k < idtable.size; k++)
				{
					if (idtable.table[k].iddatatype == IT::IDDATATYPE::SHR && idtable.table[k].value.vint == value && idtable.table[k].idtype == IT::L)
					{
						LT::Entry entryLT = LT::writeEntry(entryLT, LEX_LITERAL, k, line);
						LT::Add(lextable, entryLT);
						findSameID = true;
				...
				if (lextable.table[lextable.size - 1].lexema == LEX_OPERATOR
					&& lextable.table[lextable.size - 2].lexema == LEX_EQUAL && idtable.table[lextable.table[lextable.size - 1].idxTI].id[0] == '-')
				{
			...
			FST::FST fstLiteralInt16(word[i], FST_SHR16LIT);
			if (FST::execute(fstLiteralInt16))
			{
				...
				for (int k = 0; k < idtable.size; k++)
				{
					if (idtable.table[k].iddatatype == IT::IDDATATYPE::SHR16
						&& idtable.table[k].value.vstr.str == value && idtable.table[k].idtype == IT::L)
					{
						LT::Entry entryLT = LT::writeEntry(entryLT, LEX_LITERAL, k, line);
						LT::Add(lextable, entryLT);
						findSameID = true;
				...
				if (lextable.table[lextable.size - 1].lexema == LEX_OPERATOR && lextable.table[lextable.size - 2].lexema
					== LEX_EQUAL && idtable.table[lextable.table[lextable.size - 1].idxTI].id[0] == '-')
				{
					LT::Entry entryLT = writeEntry(entryLT, LEX_LITERAL, indexID - 1, line);
					LT::Add(lextable, entryLT, lextable.size - 1);
				}
			...
			if (FST::execute(fstLiteralChar))
			{
				...
				if (length != 3 || word[i][0] != '\'' || word[i][2] != '\'')
				{
					Log::WriteError(log, Error::geterrorin(202, line, position));
					continue;
				}

				char charValue = word[i][1];

				for (int k = 0; k < idtable.size; k++)
				{
					if (idtable.table[k].iddatatype == IT::IDDATATYPE::CHR
						&& idtable.table[k].value.vchar == charValue && idtable.table[k].idtype == IT::L)
					{
						LT::Entry entryLT = LT::writeEntry(entryLT, LEX_LITERAL, k, line);
						LT::Add(lextable, entryLT);
			...
			FST::FST fstLiteralBinary(word[i], FST_BINARYLIT);
			if (FST::execute(fstLiteralBinary))
			{
				int length = _mbslen(word[i]);

				if (length != 10 || word[i][0] != '0' || (word[i][1] != 'b' && word[i][1] != 'B'))
				{
					Log::WriteError(log, Error::geterrorin(202, line, position));
					continue;
				}
				...
				for (int k = 0; k < idtable.size; k++)
				{
					if (idtable.table[k].iddatatype == IT::IDDATATYPE::SHR2
						&& idtable.table[k].value.vint == value && idtable.table[k].idtype == IT::L)
					{
						LT::Entry entryLT = LT::writeEntry(entryLT, LEX_LITERAL, k, line);
						LT::Add(lextable, entryLT);
			...
			if (FST::execute(fstOperator))
			{
				...
				switch (word[i][0])
				{
					...
					case '%':
						entryLT.priority = 3;
						entryLT.op = LT::operations::OPROC;
						break;
				}
