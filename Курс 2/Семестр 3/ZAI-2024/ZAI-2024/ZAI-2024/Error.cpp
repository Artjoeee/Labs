...
namespace Error 
{
	ERROR errors[ERROR_MAX_ENTRY] = {
		...
		ERROR_ENTRY(114, "Ошибка при создании выходного файла (-out)"), ERROR_ENTRY_NODEF(115),
		...
		ERROR_ENTRY(307, "Семантическая ошибка: Кол-во параметров превышает максимальное значение(3)"),
		ERROR_ENTRY(308, "Семантическая ошибка: Кол-во ожидаемых и переданных в функцию параметров не совпадают"),
		...
		ERROR_ENTRY(313, "Семантическая ошибка: Недопустимый размер целочисленного литерала"),
		...
		ERROR_ENTRY(318, "Семантическая ошибка: Попытка деления на ноль"),
	...
	ERROR geterror(int id) 
	{
		...
		{
			cout << "\nОшибка " << id << ": " << errors[id].message << endl;

			return errors[id];
		}
		else 
		{
			cout << "\nОшибка " << 0 << ": " << errors[0].message << endl;
			return errors[0];
		}
	}
	...
	ERROR geterrorin(int id, int line = -1, int col = -1) 
	{
		...
		if (id > 0 && id < ERROR_MAX_ENTRY) 
		{
			...
			cout << "\nОшибка " << e.id << ": " << e.message << " cтрока " << e.inext.line << " позиция " << e.inext.col << endl;
