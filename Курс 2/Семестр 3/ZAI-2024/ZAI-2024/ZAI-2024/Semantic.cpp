		...
		switch (lextable.table[i].lexema)
		{
			case LEX_DIRSLASH: 
			{
				if (lextable.table[i + 1].lexema == LEX_LITERAL) 
				{
					...
					{
						throw Error::geterrorin(318, lextable.table[i + 1].sn, 0);
					}
				}
						...
						if (lextable.table[k].idxTI != LT_TI_NULLIDX)
						{
							if (!ignore)
							{
								if (lextable.table[k].lexema == LEX_ID || lextable.table[k].lexema == LEX_LITERAL) 
								{
									...
									if (righttype == IT::IDDATATYPE::SHR16) 
										righttype = IT::IDDATATYPE::SHR;
			...
			case LEX_LOGOPERATOR:
			{
				...
				{
					if (idtable.table[lextable.table[i - 1].idxTI].iddatatype != IT::IDDATATYPE::SHR)
						flag = false;
				}
